...
Vous trouverez dans ces pages, les informations qui vous permettront de découvrir la clinique et son équipe, ainsi que tous les renseignements nécessaires pour nous contacter et venir nous rendre visite.</p>
      <!--<p class="lead mb-0"><a href="<?=ROOT_URL?>blog_chapters.html" class="text-black fw-bold"> Voir les articles...</a></p>-->
    </div></div></br>
        ...
        <div class="col l2 m6 s6">
          <div class="card hoverable">
            ...
            <div class="card-image waves-effect waves-block waves-light">
    					<img src="<?=ROOT_URL?>static/img/posts/<?= $oAnimal->photo?>" class="activator" alt="<?= $oAnimal->nom ?>">
    				</div>
            <div class="card-content">
              <span class="card-title activator grey-text text-darken-4"><i class="material-icons right">more_vert</i></span>
              <p><a href="<?=ROOT_URL?>blog_post_<?=$oAnimal->id?>.html">Voir le dossier</a></p>
            </div>
            <div class="card-reveal">
    					<span class="card-title grey-text text-darken-4"><?= $oAnimal->nom ?><i class="material-icons right">close</i></span>
    					<p>Date de naissance : <?= preg_replace("/<img[^>]+\>/i", "", nl2br(mb_strimwidth($oAnimal->dateNaissance, 0, 800, '...'))); ?></p>
    				</div>
          </div>
        </div>
      <?php endforeach ?>
