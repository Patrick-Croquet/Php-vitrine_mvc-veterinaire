...
<main>
    <div class="container">
        ...
        <?php if (empty($this->oAnimal)): ?>
            <h1>cet dossier n'existe pas !</h1>
        <?php else: ?>
            ...
            <h4 id="comment_ink">Notes de Vétérinaire :</h4>
            <?php if (empty($this->oComments)): ?>
                <p class="bold">Aucune note n'a été publié... Soyez le premier!</p>
            <?php else: ?>
                ...
                <?php foreach ($this->oComments as $oComment): ?>
                    ...
                    <?php if (!empty($_SESSION['is_admin'])): ?>
                        <a href="<?=ROOT_URL?>?p=admin&amp;a=deleteComment&amp;id=<?=$oComment->id?>&amp;animalId=<?=$this->oAnimal->id?>"><button class="btn light-green waves-effect waves-light">Supprimer</button></a>
                    <?php endif ?>
                    ...
                    <?php if(!empty($_SESSION['is_user'])): ?>
                        ...
                        <form class="vote-form" action="blog_signal_<?=$this->oAnimal->id?>_<?=$oComment->id?>_1.html" method="POST">
            ...
            <?php if(empty($_SESSION['is_user']) && empty($_SESSION['is_admin'])): ?>
                <a href="<?=ROOT_URL?>?p=blog&amp;a=login"><button class="btn light-green lighten-2 waves-effect waves-light">Se connecter pour ajouter un note</button></a>
                <br><br>
            <?php else: ?>
                ...
                <form method="post">
                    <div class="row">
                        ...
                        <div class="col s12">
                            <button type="submit" name="submit_comment" class="btn deep-orange waves-effect waves-light">
                                Ajouter une note
                            </button>
                        </div>
                    </div>
